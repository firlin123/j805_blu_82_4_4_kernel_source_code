...
inline static int adopt_CAMERA_HW_Open(void)
{
    ...
    if (atomic_read(&g_CamHWOpend) == 0  ) {
        ...
        if (g_pSensorFunc) {
         ...
         //++++rgk add for AF enable stable by anxiang.xiao 20140701    
            ...
            else
            {
                ...
                for ( i = KDIMGSENSOR_INVOKE_DRIVER_0 ; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS ; i++ ) {
                    if ( g_bEnableDriver[i] && (g_invokeSocketIdx[i] == DUAL_CAMERA_MAIN_SENSOR)) 
                    {
...
********************************************************************************/
//modify by jinmin.xu for auto flashlight
