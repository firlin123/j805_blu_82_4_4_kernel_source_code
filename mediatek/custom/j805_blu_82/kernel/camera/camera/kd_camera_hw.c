    ...
    if (On) {
		...
		else if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_IMX179_MIPI_RAW_2LANE,currSensorName)))			  
		{
	        ...
	        if (GPIO_CAMERA_INVALID != pinSet[pinSetIdx][IDX_PS_CMRST])
			{
				...
				if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_OFF])){PK_DBG("[CAMERA LENS] set gpio failed!! \n");}
				mdelay(10);
				if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMPDN],pinSet[pinSetIdx][IDX_PS_CMPDN+IDX_PS_ON])){printk("imx179_mipi_raw_2lane set IDX_PS_CMPDN failed!! \n");}
				...
				if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_OFF])){PK_DBG("[CAMERA SENSOR] set gpio failed!! \n");}
				mdelay(10);
				if(mt_set_gpio_out(pinSet[pinSetIdx][IDX_PS_CMRST],pinSet[pinSetIdx][IDX_PS_CMRST+IDX_PS_ON])){PK_DBG("imx179_mipi_raw_2lane set IDX_PS_CMRST failed!! \n");}
				mdelay(1);
			}
			mdelay(10);
			//DVDD
		...
		else if (currSensorName && ((0 == strcmp(SENSOR_DRVNAME_GC2035_YUV,currSensorName) ) ||(0 == strcmp(SENSOR_DRVNAME_GC0329_YUV,currSensorName) )))
        {
