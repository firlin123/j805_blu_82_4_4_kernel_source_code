...
#define HELITAI_SENSOR_ID		0  //add by yangjuwei
#define HELITAI_SENSOR_ID_2		1  //add by yangjuwei
#define DIJIN_SENSOR_ID 			2  //add by yangjuwei
...
#else
static struct touch_vitual_key_map_t touch_key_point_maping_array[3] ;
#endif
                                     ...
                                     struct device_attribute *attr, char *buf)
{
	...
	if (sensor_id  == HELITAI_SENSOR_ID || sensor_id  == HELITAI_SENSOR_ID_2)
	{
		return sprintf(buf, "TP IC :%s,TP module :%s,ic firmware vesion: 0x%x,file firmware version: 0x%x,",ic_g,"HeLiTai",ic_fw_version_g,file_fw_version_g);
	    
	}
	else if (sensor_id == DIJIN_SENSOR_ID) 
	{
		return sprintf(buf, "TP IC :%s,TP module :%s,ic firmware vesion: 0x%x,file firmware version: 0x%x,",ic_g,"DiJin",ic_fw_version_g,file_fw_version_g);
	}
	else
		return sprintf(buf, "TP IC :%s,TP module :%s,sensor id:%d,ic firmware vesion: 0x%x,file firmware version: 0x%x,",ic_g,"unkown",sensor_id,ic_fw_version_g,file_fw_version_g);
}
...
// for compatable for HD and QHD LCM
#if 1
	
  GTP_MAX_HEIGHT = DISP_GetScreenHeight();
  GTP_MAX_WIDTH = DISP_GetScreenWidth();

  for (i = 0; i < TPD_KEY_COUNT; i++)
  {
  	tpd_keys_dim_local[i][VKEY_Y_INDEX] = GTP_MAX_HEIGHT + tpd_keys_dim_local[i][VKEY_HEIGHT_INDEX] ;
	   printk("\n@@@ 1886 v_key  x : %d,y : %d \n", tpd_keys_dim_local[i][VKEY_X_INDEX], tpd_keys_dim_local[i][VKEY_Y_INDEX]);
...
// add start by anxiang.xiao
	if (sensor_id  == HELITAI_SENSOR_ID || sensor_id  == HELITAI_SENSOR_ID_2)
	{
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,ic firmware version:0x%x,file firmware version: 0x%x,",ic_g,"HeLiTai",client->addr,ic_fw_version_g,file_fw_version_g);
	}
	else if (sensor_id == DIJIN_SENSOR_ID) 
	{
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,ic firmware version:0x%x,file firmware version: 0x%x,",ic_g,"DiJin",client->addr,ic_fw_version_g,file_fw_version_g);
	}
	else
		len = sprintf(tp_info, "TP IC :%s,TP module :%s,TP I2C adr : 0x%x,sensor id :%d,ic firmware version:0x%x,file firmware version: 0x%x,",ic_g,"Unkown",client->addr,sensor_id,ic_fw_version_g,file_fw_version_g);
    ...
    do
    {
        ...
        if (key_value || pre_key)
        {
            for (i = 0; i < GTP_MAX_KEY_NUM; i++)
            {
                ...
                {
                    input_x = tpd_keys_dim_local[i][VKEY_X_INDEX];
                    input_y = tpd_keys_dim_local[i][VKEY_Y_INDEX];
                    // printk("\n 2607 #### key_value = %d, button =x = %d, y= %d\n", key_value, input_x, input_y);
