...
void hdmi_state_callback(HDMI_STATE state)
{
    ...
    switch (state)
    {
        ...
        {
//if ((hdmi_params->cabletype == MHL_CABLE)||(hdmi_params->cabletype == MHL_2_CABLE))
        {
	//#ifdef CONFIG_HAS_SBSUSPEND
          //  sb_plug_in();
	//#endif
...
static int hdmi_probe(struct platform_device *pdev)
{
    ...
    printk(KERN_ALERT "[hdmi][%s] current=0x%08x\n", __func__, (unsigned int)current);
...
static int __init hdmi_init(void)
{
    ...
    printk(KERN_ALERT "Output mode is %s\n", (hdmi_params->output_mode == HDMI_OUTPUT_MODE_DPI_BYPASS) ? "HDMI_OUTPUT_MODE_DPI_BYPASS" : "HDMI_OUTPUT_MODE_LCD_MIRROR");
    ...
    int tmp_boot_mode = get_boot_mode();
       ...
       printk(KERN_ALERT "%s-%d, boot_mode=%d\n", __FUNCTION__, __LINE__, tmp_boot_mode);
